...
:project
for /f "skip=1" %%A in (
'wmic product where "Name like '%%Microsoft Project%%'" get installstate'
) do (
	if %%A==5 goto finished
	)
	echo.Project not installed. Proceeding to install Microsoft Project 2016.
	goto check_iso

REM | Check if ISO can be found. If not, echo an error message and go to end of script.
:check_iso
IF NOT EXIST \\rnas\admin\iso\custom\SW_DVD5_Project_Pro_2016_64Bit_English_MLF_X20-42683.iso (
echo ISO does not exist or cannot be reached
goto finished
)

REM | ISO variable
SET NASISO=\\rnas\admin\iso\custom\SW_DVD5_Project_Pro_2016_64Bit_English_MLF_X20-42683.iso
SET ISO=C:\SW_DVD5_Project_Pro_2016_64Bit_English_MLF_X20-42683.iso
...
For /F "Skip=1 Delims=" %%A In ('
    "WMIC LogicalDisk Where (VolumeName='SW_DVD5_Project_') Get Name"
') Do For %%B In (%%A) Do Set "ISODRIVE=%%B"

REM | Run installer with customisation adminfile
echo Installing Microsoft Project 2016...
...
cscript %OfcPath%\ospp.vbs /inpkey:YG9NW-3K39V-2T3HJ-93F3Q-G83KT
