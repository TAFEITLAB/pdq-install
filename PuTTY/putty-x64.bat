...
REM | PuTTY x64
REM | =========
REM | Purpose: Silently install PuTTY 64-bit via PDQ Deploy.
REM | Requirements: 
REM |   - Place in same directory as binary.
REM |   - Ensure script matches binary name.
REM | Author: Tim Dunn
...
set BINARY=putty-64bit-0.70-installer.msi
set FLAGS=ALLUSERS=1 /qn /norestart /log output.log
...
wmic process where name="putty.exe" call terminate 2>NUL
